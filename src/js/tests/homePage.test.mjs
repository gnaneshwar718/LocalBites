...
describe('HomePage DOM Unit Tests', () => {
    ...
    global.fetch = jest.fn((url) => {
      ...
      if (url.includes('footer.html')) {
        return Promise.resolve({
          text: () =>
            Promise.resolve(
              '<footer><p id="footer-copyright"></p><a href="mailto:#" id="footer-contact-link">Contact</a></footer>'
            ),
        });
      }
      if (url.includes('faq.html')) {
        return Promise.resolve({
          text: () =>
            Promise.resolve(
              '<section id="faq"><h2>Questions</h2><div class="faq-list"><faq-item question="Q1" answer="A1"></faq-item></div><a href="mailto:#" id="faq-contact-link">CONTACT US</a></section>'
            ),
        });
      }
  ...
  test('AppHeader should toggle hamburger menu and contain navigation links', async () => {
    ...
    expect(container.querySelector('a[href="/budget"]')).toHaveTextContent(
      'Budget Planner'
    );
    expect(container.querySelector('a[href="/auth"]')).toHaveTextContent(
      'Profile'
    );
  });
  ...
  test('AppFooter should render correctly and have dynamic content', async () => {
    ...
    await import(
      '../components.js?t=' + (Date.now() + CACHE_BUST_OFFSET_FOOTER)
    );
    await waitFor(() =>
    ...
    await waitFor(() =>
      expect(copyright).toHaveTextContent(
        'Copyright © 2026 by LocalBites, All Rights Reserved'
      )
    );
  });
